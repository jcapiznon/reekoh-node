...
class Service extends EventEmitter {
  ...
  logException (err) {
    return new Promise((resolve, reject) => {
      if(!(err instanceof Error)) return reject(new Error('Please specify a valid error to log.'))
  ...
  pipe (data, result) {
    return new Promise((resolve, reject) => {
      ...
      if (process.env.OUTPUT_SCHEME === 'MERGE') {
        ...
        let outputData = Object.assign(data[process.env.OUTPUT_NAMESPACE], result)
        ...
        async.each(this.outputPipes, (outputPipe, done) => {
          this.queues[outputPipe].publish(result)
            .then(() => {
              console.log(`message written to queue ${outputPipe}`)
              done()
            })
